...
RUN	apk --no-cache --no-progress upgrade -f && \
	apk --no-cache --no-progress add nginx php5 php5-fpm

RUN	echo 'pid /var/run/nginx.pid;' > /etc/nginx/modules/pid.conf && \
	test ! -x /usr/bin/php && ln -s /usr/bin/php5 /usr/bin/php

COPY	nginx_default_server.conf /etc/nginx/conf.d/default.conf
COPY	php-fpm-www.conf /etc/php5/
COPY	php-cli.ini /etc/php5/php-cli.ini
